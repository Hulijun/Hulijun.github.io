...
//4.要左右切换，定位的时候left的设置也是有点复杂的..要考虑方向等情况
//图片组相对原始左移dist px距离
function goLeft(elem,dist){ 
	if(dist == 955){ //第一次时设置left为0px 或者直接使用内嵌法 style="left:0;"
	  elem.style.left = "0px";
	}
	...
	for(var i=0;i<= dist/5;i++){ //这里设定缓慢移动，5阶每阶191px
	  (function(_i){ 
			var pos = parseInt(elem.style.left); //获取当前left
			setTimeout(function(){ 
				  pos += (toLeft)? -(_i * 5) : (_i * 5); //根据toLeft值指定图片组位置改变
				  //console.log(pos);
